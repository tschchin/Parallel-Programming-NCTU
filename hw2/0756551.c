...
//#pragma omp parallel for private(i)
  ...
  for (it = 1; it <= NITER; it++) {
    ...
    norm_temp1 = 0.0;
    norm_temp2 = 0.0;
#pragma omp parallel for private(j) shared(x,z) reduction(+:norm_temp1,norm_temp2)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      norm_temp1 = norm_temp1 + x[j]*z[j];
      norm_temp2 = norm_temp2 + z[j]*z[j];
    }
    ...
    // Normalize z to obtain x
    //---------------------------------------------------------------------
#pragma omp parallel for private(j) shared(norm_temp2)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      x[j] = norm_temp2 * z[j];
    }
                      ...
                      double *rnorm)
{
  ...
  //---------------------------------------------------------------------
#pragma omp parallel for private(j) shared(q,z,r,p)
  for (j = 0; j < naa+1; j++) {
    q[j] = 0.0;
    z[j] = 0.0;
    r[j] = x[j];
...
#pragma omp parallel for private(j,k,sum) shared(q)
    for (j = 0; j < lastrow - firstrow + 1; j++) {
      sum = 0.0;
      for (k = rowstr[j]; k < rowstr[j+1]; k++) {
...
#pragma omp parallel for private(j) reduction(+:rho)   
    ...
    // p = r + beta*p
    //---------------------------------------------------------------------
#pragma omp parallel for private(j) shared(r,q) 
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      p[j] = r[j] + beta*p[j];
    }
